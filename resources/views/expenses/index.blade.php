    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        ...
        <form action="{{ route('expenses.index') }}" method="GET" class="d-flex align-items-center w-100 w-md-auto">
            <div class="input-group shadow-sm">
                <input type="text" name="search" class="form-control" placeholder="Search Expense..." value="{{ request('search') }}">
        ...
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    ...
                    <th class="py-3"><a href="{{ sortUrl('Date') }}" class="text-dark text-decoration-none fw-bold">Date{!! sortIcon('Date') !!}</a></th>
                    <th class="py-3"><a href="{{ sortUrl('JobNo') }}" class="text-dark text-decoration-none fw-bold">Job No{!! sortIcon('JobNo') !!}</a></th>
                    <th class="py-3"><a href="{{ sortUrl('CompanyName') }}" class="text-dark text-decoration-none fw-bold">Company{!! sortIcon('CompanyName') !!}</a></th>
                    <th class="py-3"><a href="{{ sortUrl('Total') }}" class="text-dark text-decoration-none fw-bold">Total{!! sortIcon('Total') !!}</a></th>
                    <th class="py-3"><a href="{{ sortUrl('Currency') }}" class="text-dark text-decoration-none fw-bold">Curr{!! sortIcon('Currency') !!}</a></th>
                    ...
                    <tr class="table-row">
                        ...
                        <td class="py-3 text-center">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('expenses.show', $expense->id) }}" class="btn btn-outline-info" title="View">
                                    <i class="bi bi-eye">Show</i>
                                </a>
                                <a href="{{ route('expenses.edit', $expense->id) }}" class="btn btn-outline-primary" title="Edit">
